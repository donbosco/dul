...
public class Crypt
{
  ...
  private static int[] des_set_key(byte[] paramArrayOfByte)
  {
    ...
    for (int i1 = 0; i1 < 16; i1++)
    {
      ...
      i &= 268435455;
      j &= 268435455;
      int k = skb[0][(i & 0x3F)] | skb[1][(i >>> 6 & 0x3 | i >>> 7 & 0x3C)] | skb[2][(i >>> 13 & 0xF | i >>> 14 & 0x30)] | skb[3][(i >>> 20 & 0x1 | i >>> 21 & 0x6 | i >>> 22 & 0x38)];
      int m = skb[4][(j & 0x3F)] | skb[5][(j >>> 7 & 0x3 | j >>> 8 & 0x3C)] | skb[6][(j >>> 15 & 0x3F)] | skb[7][(j >>> 21 & 0xF | j >>> 22 & 0x30)];
      arrayOfInt1[(n++)] = ((m << 16 | k & 0xFFFF) & 0xFFFFFFFF);
      k = k >>> 16 | m & 0xFFFF0000;
      k = k << 4 | k >>> 28;
      arrayOfInt1[(n++)] = (k & 0xFFFFFFFF);
    }
    return arrayOfInt1;
  }
  ...
  private static final int D_ENCRYPT(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5, int[] paramArrayOfInt)
  {
    ...
    i = i >>> 4 | i << 28;
    paramInt1 ^= (SPtrans[1][(i & 0x3F)] | SPtrans[3][(i >>> 8 & 0x3F)] | SPtrans[5][(i >>> 16 & 0x3F)] | SPtrans[7][(i >>> 24 & 0x3F)] | SPtrans[0][(j & 0x3F)] | SPtrans[2][(j >>> 8 & 0x3F)] | SPtrans[4][(j >>> 16 & 0x3F)] | SPtrans[6][(j >>> 24 & 0x3F)]);
    return paramInt1;
  }
  ...
  private static final int[] body(int[] paramArrayOfInt, int paramInt1, int paramInt2)
  {
    ...
    for (int m = 0; m < 25; m++)
    {
      ...
      {
        i = D_ENCRYPT(i, j, n, paramInt1, paramInt2, paramArrayOfInt);
        j = D_ENCRYPT(j, i, n + 2, paramInt1, paramInt2, paramArrayOfInt);
      }
      k = i;
      i = j;
      j = k;
  ...
  {
    String str = Integer.toHexString(new Random().nextInt() & 0xFFFF);
    return crypt(str, paramString);
  }
  ...
  public static final String crypt(String paramString1, String paramString2)
  {
    ...
    int i = con_salt[c1];
    int j = con_salt[c2] << 4;
    byte[] arrayOfByte1 = new byte[8];int k = 0;
    for ( k = 0; k < arrayOfByte1.length; k++)
      arrayOfByte1[k] = 0;
    for (k = 0; (k < arrayOfByte1.length) && (k < paramString2.length()); k++)
    {
      int m = paramString2.charAt(k);
      arrayOfByte1[k] = ((byte)(m << 1));
    }
    ...
    while (n < 13)
    {
      ...
      while (i3 < 6)
      {
        ...
        localStringBuffer.setCharAt(n, (char)cov_2char[i4]);
        i3++;
      }
      n++;
    }
    ...
    throws IOException
  {
    ...
    System.out.println(generate(localStringBuffer.toString()));
  }
}
